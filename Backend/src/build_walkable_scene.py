...
3MF house mesh into a fully configured, walkable Blender scene.
...
Pipeline Steps:
    ...
    6. Save & Exit            - Write the .blend file and quit.
...
def _parse_3mf_to_obj(filepath: str, obj_path: str, mtl_path: str) -> None:
    ...
    with open(obj_path, 'w') as of:
        ...
        for obj_el in root.findall('.//m:object', ns):
            ...
            current_mat  = None
            base_offset  = global_vertex_offset  

            of.write(f"usemtl mat_default\n")  
            current_mat = 'mat_default'

            for tri in triangles:
                v1 = int(tri.get('v1')) + base_offset + 1  
...
COLOR_PALETTE = {
    ...
    'bed':              (0.20, 0.02, 0.02),   
    'sofa':             (0.01, 0.03, 0.08),   
    'couch':            (0.01, 0.03, 0.08),   
    'table':            (0.05, 0.03, 0.01),   
    'counter':          (0.05, 0.10, 0.10),   
    'chair':            (0.08, 0.05, 0.02),   
    'wardrobe':         (0.05, 0.02, 0.01),   
    'desk':             (0.08, 0.05, 0.02),   
    'toilet':           (0.50, 0.50, 0.55),   
    'bathtub':          (0.45, 0.50, 0.55),   
    'sink':             (0.40, 0.45, 0.50),   
    'shelf':            (0.15, 0.08, 0.03),   

    'wall':             (0.15, 0.14, 0.12),   
    'floor':            (0.15, 0.08, 0.04),   
    'ceiling':          (0.30, 0.30, 0.25),   
    'door':             (0.10, 0.05, 0.02),   
    'window':           (0.10, 0.15, 0.20),   

    'default_furniture':(0.08, 0.05, 0.02),   
    'default_house':    (0.15, 0.14, 0.12),   
}
...
def build_pbr_material(obj: bpy.types.Object) -> None:
    ...
    if rgb == COLOR_PALETTE['wall'] or rgb == COLOR_PALETTE['default_house']:
        rgb_outer = rgb
        rgb_inner = (rgb[0] * 0.75, rgb[1] * 0.75, rgb[2] * 0.75)  
...
def add_interior_lighting(objects: list) -> None:
    ...
    for obj in objects:
        ...
        obj_cz = max(v.z for v in verts) + 1.5   
...
def swap_furniture_with_assets() -> None:
    ...
    for item in furniture_list:
        ...
        elif "cabinet" in ftype: asset_file = "cabinet.glb" 
